// windows/force_link.c
//
// R131 — MSVC equivalent of iOS's SymEngineBridge.m + DummySymbols.c
// trick. Belt + braces for keeping the `flutter_symengine_*` symbols
// alive against MSVC's dead-code stripping:
//
//   1. CMakeLists.txt passes `/INCLUDE:<symbol>` for each entry point
//      (link-time directive).
//   2. This function takes the address of every entry point and stores
//      it through a volatile sink so the compiler can't const-fold the
//      references away. Called from `SymbolicMathBridgePlugin::Register`.
//
// Either mechanism alone usually suffices on MSVC; both together is
// defense-in-depth, mirroring the iOS pattern.
